...
import { MeetingTag, TAGS_UPDATED_EVENT, tagService } from "@/services/tagService";
...
export function MeetingTagsButton({ meetingId }: MeetingTagsButtonProps) {
  ...
  useEffect(() => {
    ...
    return () => {
      cancelled = true;
      window.removeEventListener(TAGS_UPDATED_EVENT, onTagsUpdated);
    };
  }, [meetingId]);
  ...
  return (
    <Popover open={open} onOpenChange={setOpen}>
      <PopoverTrigger asChild>
        ...
        >
          {meetingTags.length > 0 ? <TagColorDot color={meetingTags[0].color} /> : <TagIcon size={18} />}
      ...
      <PopoverContent align="start" className="w-auto p-0 border-0 shadow-none bg-transparent">
        <TagPickerPopover meetingId={meetingId} />
      </PopoverContent>
    </Popover>
  );
}
